...
The goal of this project is to simulate the demand prediction and energy transaction process in which CPS Energy engages on a daily basis. Several assumptions have been made and are noted as such. They can be tuned if appropriate.

CPS is owned by the City of San Antonio and is responsible for its own customers and service area shown below. Excess power generated by CPS can be sold via the ERCOT market. This revenue benefits CPS and its customers and is known as off-system sales. 
...
The existing CPS data was then augmented with a couple of new features based only on the date: **dayOfWeek** and **weekend.flag**. 
